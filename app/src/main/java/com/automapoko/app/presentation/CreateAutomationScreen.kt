...
fun CreateAutomationScreen(
    ...
    Scaffold(
        ...
        Column(Modifier.fillMaxSize().padding(padding).padding(16.dp), verticalArrangement = Arrangement.spacedBy(16.dp)) {
            ...
            if (selectedTrigger == TriggerType.BLUETOOTH) {
                ...
                OutlinedTextField(value = wifiSsid, onValueChange = { wifiSsid = it }, label = { Text("Nome da Rede Wi-Fi") }, modifier = Modifier.fillMaxWidth())
            }
            ...
            Button(
                onClick = {
                    ...
                    val configJson = when (selectedTrigger) {
                        ...
                        TriggerType.LOCATION -> Json.encodeToString(TriggerConfig.Location(0.0, 0.0, 100f, GeofenceTransitionType.ENTER))
                    }
                    onSave(AutomationEntity(name = name, triggerType = selectedTrigger, triggerConfigJson = configJson, targetPackageName = app.packageName, targetAppName = app.name))
                },
    ...
    if (showDialog) {
        AlertDialog(
            ...
            title = { Text("Escolha o App") },
            text = {
                LazyColumn(Modifier.height(300.dp)) {
                    items(installedApps) { app ->
                        ListItem(headlineContent = { Text(app.name) }, modifier = Modifier.clickable { selectedApp = app; showDialog = false })
                    }
                }
            },
            confirmButton = {}
